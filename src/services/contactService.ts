...
class ContactService {
  ...
  async previewCsvImport(file: File): Promise<any> {
    return this.uploadCsvFile('/contacts/csv/preview', file);
  }

  async saveCsvMapping(data: CsvMappingPayload): Promise<any> {
    return apiClient.post<CsvMappingPayload, any>('/contacts/csv/mappings', data);
  }

  async listCsvMappings(): Promise<any> {
    return apiClient.get<any>('/contacts/csv/mappings');
  }

  async getCsvMapping(id: string): Promise<any> {
    return apiClient.get<any>(`/contacts/csv/mappings/${id}`);
  }
  ...
  async executeCsvImport(
    ...
    return this.uploadCsvFile('/contacts/csv/execute', file, extraFields);
  }
