...
class InvoiceService {
  ...
  async previewCsvImport(file: File): Promise<any> {
    return this.uploadCsvFile('/invoices/csv/preview', file);
  }

  async saveCsvMapping(data: CsvMappingPayload): Promise<any> {
    return apiClient.post<CsvMappingPayload, any>('/invoices/csv/mappings', data);
  }

  async listCsvMappings(): Promise<any> {
    return apiClient.get<any>('/invoices/csv/mappings');
  }

  async getCsvMapping(id: string): Promise<any> {
    return apiClient.get<any>(`/invoices/csv/mappings/${id}`);
  }
  ...
  async executeCsvImport(
    ...
    return this.uploadCsvFile('/invoices/csv/execute', file, extraFields);
  }
