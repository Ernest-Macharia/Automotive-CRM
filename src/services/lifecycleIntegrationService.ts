...
public async canStageAutoTransition(stage: string, document: any): Promise<boolean> {
...
// Method to get detailed current stage information
  public async getCurrentStageDetails(opportunityId: string): Promise<{
    ...
    try {
      ...
      const canAutoTransition = await this.canStageAutoTransition(currentStage.stage, currentStage.document);
  ...
  async createDocumentAndUpdateWorkflow(
    stage: string,
    opportunityId: string,
    data: any,
    userId?: string
    ...
    try {
      ...
      // Auto-approve checklists based on client signature
      const enhancedData = {
        ...data,
        createdBy: userId || 'system',
        createdAt: new Date().toISOString(),
        autoGenerated: false // Except for invoice
      };
      
      switch (stage) {
        case 'prechecklist':
        case 'pre_checklist':
          // Auto-approve pre-checklist if client signature exists
          const preApproved = this.shouldAutoApprovePreChecklist(data);
          const preChecklistData = {
            ...enhancedData,
            approved: preApproved,
            approvedAt: preApproved ? new Date().toISOString() : undefined,
            approvedBy: preApproved ? (userId || 'system-auto') : undefined,
            status: 'completed'
          };
        ...
        case 'jobcard':
        case 'job_card':
          // Job card is created manually, status starts as 'pending'
          document = await jobCardService.createJobCard(enhancedData);
          break;
          
        case 'postchecklist':
        case 'post_checklist':
          // Auto-approve post-checklist if client signature exists
          const postApproved = this.shouldAutoApprovePostChecklist(data);
          const postChecklistData = {
            ...enhancedData,
            approved: postApproved,
            approvedAt: postApproved ? new Date().toISOString() : undefined,
            approvedBy: postApproved ? (userId || 'system-auto') : undefined,
            status: 'completed'
          };
        ...
        case 'invoice':
          // Auto-generate invoice after post-checklist
          const invoiceData = {
            ...enhancedData,
            autoGenerated: true,
            status: 'draft'
      ...
      if (stage === 'prechecklist' || stage === 'pre_checklist') {
        // Auto-transition to jobcard after pre-checklist (regardless of approval)
        shouldTransition = true;
        nextStage = 'jobcard';
      } 
      else if (stage === 'jobcard' || stage === 'job_card') {
        // Only transition when job card is completed
        shouldTransition = document?.status === 'completed';
        nextStage = 'postchecklist';
      }
      else if (stage === 'postchecklist' || stage === 'post_checklist') {
        // Auto-transition to invoice after post-checklist
        shouldTransition = true;
        nextStage = 'invoice';
      }
      
      if (shouldTransition && nextStage) {
        await this.transitionToStage(opportunityId, nextStage, {
          ...
          metadata: {
            ...
            fromStage: stage,
            toStage: nextStage,
            triggeredBy: 'document-completion'
          }
        });
      }
      ...
      if (canAutoTransition) {
        ...
        if (currentIndex < pattern.length - 1) {
          ...
          if (workOrders.length > 0) {
            ...
            await workOrderService.updateWorkOrder(workOrder._id, {
              currentStage: 'invoice' as const, // Add 'as const' to ensure it's the literal type
              updatedAt: new Date().toISOString()
            });
          }
...
async isJobCardProperlyCreated(opportunityId: string): Promise<{
  ...
  try {
    ...
    // assignedTo may be string OR object depending on API population
    const assignedToId =
      typeof jobCard.assignedTo === 'string'
        ? jobCard.assignedTo
        : ((jobCard.assignedTo as any)?._id ?? (jobCard.assignedTo as any)?.id ?? '');
    ...
    return {
      hasJobCard: true,
      isAssigned: isAssignedBool, // ✅ boolean guaranteed
      jobCard,
      reason: isAssignedBool ? 'Job card properly assigned' : 'Job card not properly assigned',
    };
  } catch (error) {
...
// Add these private methods to LifecycleIntegrationService class
  ...
  public async oneClickCompleteStage(
    ...
    try {
      ...
      if (!document?._id) {
        return {
          success: false,
          message: `No ${stage} document found. Please create it first to proceed.`
        };
      }
      ...
      if (nextStage) {
        ...
        // Only auto-create invoice (derived from opportunity/sales order)
        await this.autoCreateNextDocument(opportunityId, nextStage, document);

        // Keep Work Order current stage in sync
        await this.updateWorkOrderStage(opportunityId, nextStage);

        return {
          success: true,
          message: `Stage "${stage}" completed. Moved to "${nextStage}".`,
          nextStage
        };
      }

      // If there is no next stage, complete work order
      await this.completeWorkOrder(opportunityId);
      return {
        success: true,
      ...
      return {
        success: false,
        message: error instanceof Error ? error.message : 'One-click completion failed'
      };
    }
  }
  ...
  public async getEnhancedWorkflowUIWithAutoTransition(opportunityId: string): Promise<any> {
    ...
    const enhancedStages = workflowUI.stages.map((stage: any) => {
      const canAutoTransition = this.canStageAutoTransition(stage.stage, stage.document);
